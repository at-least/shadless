---
title: "Components"
sort_by: "weight"
---

# Components

51 components ported · 15 not ported (they need React, or upstream removed them) · 7 guides.

New here? Read the [Introduction](/guides/introduction) to learn what shadless is and why it exists.

- [accordion](/components/accordion)
- [alert](/components/alert)
- [alert-dialog](/components/alert-dialog)
- [aspect-ratio](/components/aspect-ratio)
- [attachment](/components/attachment)
- [avatar](/components/avatar)
- [badge](/components/badge)
- [breadcrumb](/components/breadcrumb)
- [bubble](/components/bubble)
- [button](/components/button)
- [button-group](/components/button-group)
- calendar <span class="unavailable">not available</span>
- [card](/components/card)
- [carousel](/components/carousel)
- chart <span class="unavailable">not available</span>
- [checkbox](/components/checkbox)
- [collapsible](/components/collapsible)
- combobox <span class="unavailable">not available</span>
- command <span class="unavailable">not available</span>
- [context-menu](/components/context-menu)
- data-table <span class="unavailable">not available</span>
- date-picker <span class="unavailable">not available</span>
- [dialog](/components/dialog)
- [direction](/components/direction)
- drawer <span class="unavailable">not available</span>
- [dropdown-menu](/components/dropdown-menu)
- [empty](/components/empty)
- [field](/components/field)
- form <span class="unavailable">not available</span>
- [hover-card](/components/hover-card)
- [input](/components/input)
- [input-group](/components/input-group)
- input-otp <span class="unavailable">not available</span>
- [item](/components/item)
- [kbd](/components/kbd)
- [label](/components/label)
- [marker](/components/marker)
- [menubar](/components/menubar)
- [message](/components/message)
- [message-scroller](/components/message-scroller)
- [native-select](/components/native-select)
- [navigation-menu](/components/navigation-menu)
- [pagination](/components/pagination)
- [popover](/components/popover)
- [progress](/components/progress)
- questionnaire <span class="unavailable">not available</span>
- [radio-group](/components/radio-group)
- resizable <span class="unavailable">not available</span>
- [scroll-area](/components/scroll-area)
- [select](/components/select)
- [separator](/components/separator)
- [sheet](/components/sheet)
- sidebar <span class="unavailable">not available</span>
- [skeleton](/components/skeleton)
- [slider](/components/slider)
- sonner <span class="unavailable">not available</span>
- [spinner](/components/spinner)
- [switch](/components/switch)
- [table](/components/table)
- [tabs](/components/tabs)
- [textarea](/components/textarea)
- toast <span class="unavailable">not available</span>
- [toggle](/components/toggle)
- [toggle-group](/components/toggle-group)
- [tooltip](/components/tooltip)
- typography <span class="unavailable">not available</span> — see the [Typography](/guides/typography) guide

## Guides

- [Introduction](/guides/introduction)
- [Installation](/guides/installation)
- [Dark Mode](/guides/dark-mode)
- [RTL](/guides/rtl)
- [shimmer](/guides/shimmer)
- [scroll-fade](/guides/scroll-fade)
- [Typography](/guides/typography)
